...
Future<Uint8List> _readFileIsolate(String path) async {
  final XFile file = XFile(path);
  return await file.readAsBytes();
}

Future<MediaDimensions> _processImageDimensionsIsolate(
...
Future<List<SelectedFile>?> selectMedia({
  ...
  if (multiImage) {
    ...
    return Future.wait(pickedMedia.asMap().entries.map((e) async {
      final index = e.key;
      final media = e.value;
      // Use compute for heavy file reading
      final mediaBytes = await compute(_readFileIsolate, media.path);
      final path = _getStoragePath(storageFolderPath, media.name, false, index);

      // Use compute for image processing only when dimensions are needed
      final dimensions = includeDimensions
          ? isVideo
              ? _getVideoDimensions(media.path)
              : compute(_processImageDimensionsIsolate, mediaBytes)
          : null;
  ...
  // Use compute for file reading
  final mediaBytes = await compute(_readFileIsolate, pickedMedia.path);
