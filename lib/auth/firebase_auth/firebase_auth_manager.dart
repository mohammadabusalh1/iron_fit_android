...
class FirebaseAuthManager extends AuthManager
  ...
  Future deleteUser(BuildContext context) async {
    try {
      if (!loggedIn) {
        Logger.error('Error: delete user attempted with no logged in user!');
        return;
      }
  ...
  Future updateEmail({
    ...
    try {
      if (!loggedIn) {
        Logger.error('Error: update email attempted with no logged in user!');
        return;
      }
  ...
  Future<BaseAuthUser?> signInWithGoogle(BuildContext context,
      {bool isLogin = false}) {
    if (isLogin) {
      return _signInOrCreateAccount(context, googleSignInFuncLogin, 'GOOGLE');
    }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : IronFitFirebaseUser.fromUserCredential(userCredential);
